...
-- 1. Criar tabela de clubes (se não existir)
...
-- 3. Criar tabela de jogadores (se não existir)
CREATE TABLE IF NOT EXISTS jogadores (
  id UUID PRIMARY KEY,
  nome TEXT NOT NULL,
  posicao TEXT CHECK (posicao IN ('Goleiro', 'Zagueiro', 'Lateral', 'Meio-Campo', 'Atacante')),
  numero INTEGER,
  clube_id BIGINT REFERENCES clubes,
  foto_url TEXT,
  status TEXT CHECK (status IN ('ativo', 'vendido', 'emprestado', 'suspenso')),
  valor_mercado BIGINT,
  jogos_suspensao INTEGER
);

-- 4. Criar tabela de jogos (se não existir)
CREATE TABLE IF NOT EXISTS jogos (
  id UUID PRIMARY KEY,
  clube_casa_id BIGINT REFERENCES clubes,
  clube_fora_id BIGINT REFERENCES clubes,
  gols_casa INTEGER,
  gols_fora INTEGER,
  rodada INTEGER,
  fase TEXT CHECK (fase IN ('grupos', 'repescagem', 'quartas', 'semi', 'final')),
  data TEXT,
  status TEXT CHECK (status IN ('agendado', 'realizado'))
);

-- 5. Criar tabela de estatísticas (se não existir)
CREATE TABLE IF NOT EXISTS estatisticas (
  id UUID PRIMARY KEY,
  jogador_id UUID REFERENCES jogadores,
  jogo_id UUID REFERENCES jogos,
  gols INTEGER,
...
CREATE TABLE IF NOT EXISTS transferencias (
  id UUID PRIMARY KEY,
  jogador_id UUID REFERENCES jogadores,
  clube_origem_id BIGINT REFERENCES clubes,
  clube_destino_id BIGINT REFERENCES clubes,
  valor BIGINT,
  tipo TEXT CHECK (tipo IN ('compra', 'emprestimo', 'troca', 'jogador_mais_valor')),
  jogador_troca_id UUID REFERENCES jogadores,
  data TEXT,
  status TEXT CHECK (status IN ('pendente', 'aceita', 'rejeitada')),
  mensagem TEXT
);
...
-- 9. Criar trigger para perfil automático de usuários
...
-- 10. Inserir os usuários manualmente (caso já existam)
...
-- 10. Criar funções para atualizar orçamento
...
-- 12. Criar políticas (com DROP IF EXISTS para permitir reexecução)
...
CREATE POLICY "Admin pode gerenciar usuários" ON usuarios FOR ALL USING (auth.uid() IN (SELECT id FROM usuarios WHERE role = 'admin'));
